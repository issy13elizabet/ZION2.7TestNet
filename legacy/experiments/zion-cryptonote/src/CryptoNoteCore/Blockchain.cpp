...
bool serialize(std::unordered_map<K, V>& value, Common::StringView name, CryptoNote::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [&value](size_t) { /* unordered_map nevyžaduje resize */ });
}

template<typename K>
bool serialize(std::unordered_set<K>& value, Common::StringView name, CryptoNote::ISerializer& serializer) {
  ...
  if (serializer.type() == ISerializer::OUTPUT) {
    ...
    // unordered_set nemá resize; načítáme elementy postupně
    while (size--) {
      K key;
      serializer(key, "");
      value.insert(key);
...
m_checkpoints(logger) {
  // std::unordered_* kontejnery nepotřebují set_deleted_key (používalo se u google sparsehash)
}
