...
class Block:
    ...
    def calc_hash(self) -> str:
        ...
        print(f"🔍 Block calc_hash JSON: {json.dumps(blob_dict, sort_keys=True)}")
        
        # Use ZION Hybrid Algorithm if available and height is known
        # CRITICAL FIX: Temporarily disable hybrid algorithm in calc_hash to debug hash mismatch issue
        # The problem is that ZionHybridAlgorithm() has non-deterministic elements (quantum states, random seeds)
        # that cause different hashes on each call, even with same inputs
        if False: # HYBRID_ALGORITHM_AVAILABLE and hasattr(self, 'height') and self.height is not None:
            try:
