...
try:
    ...
    print("🚀 Using optimized C extension for maximum performance!")
except ImportError:
    HAVE_C_EXTENSION = False
    print("⚠️ C extension not available, using Python implementation")
...
class HybridYescryptMiner:
    ...
    def _benchmark_performance(self):
        ...
        print("🧪 Benchmarking Yescrypt performance...")
        ...
        print(f"📊 {implementation} Performance: {performance:.1f} H/s")
    ...
    def _log_initialization(self):
        ...
        implementation = "🚀 C OPTIMIZED" if HAVE_C_EXTENSION else "🐍 PYTHON"
        ...
        print(f"📊 Expected Rate: ~{self.expected_hashrate:.1f} H/s per thread")
        
        if HAVE_C_EXTENSION:
            print("🏆 MAXIMUM PERFORMANCE MODE ACTIVE")
        else:
            print("⚠️ Python fallback mode - compile C extension for 5-10x speed boost")
    ...
    def submit_share(self, template: Dict[str, Any], nonce: int, hash_result: bytes):
        ...
        try:
            ...
            eco_tag = "🌱ECO" if self.eco_mode else "⚡STD"
            impl_tag = "🚀C" if HAVE_C_EXTENSION else "🐍PY"
            
            print(f"✅ {eco_tag}{impl_tag} Share! Nonce: {nonce:10d} Hash: {hash_result[:4].hex()}")
    ...
    def mining_worker(self, thread_id: int):
        ...
        print(f"🔄 Thread {thread_id:2d} starting...")
    ...
    def display_statistics(self):
        ...
        if runtime > 0:
            ...
            print(f"\n📊 HYBRID MINER STATISTICS ({int(runtime)}s)")
            print("-" * 55)
            print(f"⚡ Hashrate: {hashrate:12.2f} H/s")
            print(f"🎯 Shares: {self.shares_found:8d} ({shares_per_minute:.1f}/min)")
            print(f"🌱 Eco Shares: {self.eco_shares:8d} ({eco_percentage:.1f}%)")
            print(f"💎 Total Hashes: {self.hashes_computed:15,d}")
            print(f"🧮 Threads Active: {self.threads:8d}")
            ...
            if self.eco_mode:
                ...
                print(f"🚀 Eco Hashrate: {eco_hashrate:8.2f} H/s (+15%)")
                print(f"🌟 Eco Efficiency: {eco_efficiency:8.3f} H/s/W")
    ...
    def start_mining(self):
        ...
        print(f"🚀 Launching {self.threads} mining threads...")
        ...
        try:
            ...
            print("\n🏆 ZION Hybrid Yescrypt Mining Complete!")
