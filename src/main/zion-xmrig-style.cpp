...
int main(int argc, char* argv[]) {
    ...
    while (running && (cpu_miner->is_running() || gpu_miner->is_mining())) {
        ...
        if (update_counter % 2 == 0) {
            ...
            std::cout << "\033[A\033[A\033[A\033[A\033[A\033[2K";
            std::cout << "\033[A\033[2K\033[A\033[2K\033[A\033[2K";
        }
        
        print_mining_stats();
        
        // Simulate share notifications (replace with actual share callback in real implementation)
        static int last_cpu_shares = 0;
        static int last_gpu_shares = 0;
        
        auto cpu_stats = cpu_miner->get_stats();
        auto gpu_stats = gpu_miner->get_stats();
        
        if (cpu_stats.shares_found.load() > last_cpu_shares) {
            show_share_found("CPU", rand(), "0x" + std::to_string(rand()));
            last_cpu_shares = cpu_stats.shares_found.load();
        }
        
        if (gpu_stats.gpu_shares_found.load() > last_gpu_shares) {
            show_share_found("GPU", rand(), "0x" + std::to_string(rand()));
            last_gpu_shares = gpu_stats.gpu_shares_found.load();
        }
    }
    ...
    std::cout << GREEN << "* ZION mining session completed!" << RESET << std::endl;
