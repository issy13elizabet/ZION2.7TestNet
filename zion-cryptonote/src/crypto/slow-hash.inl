...
#define ctx ((struct cn_ctx *) context)
...
#else
  memcpy(&(ctx->long_state[i + 0 * 16]), &xmminput[0], 16);
  memcpy(&(ctx->long_state[i + 1 * 16]), &xmminput[1], 16);
  memcpy(&(ctx->long_state[i + 2 * 16]), &xmminput[2], 16);
  memcpy(&(ctx->long_state[i + 3 * 16]), &xmminput[3], 16);
  memcpy(&(ctx->long_state[i + 4 * 16]), &xmminput[4], 16);
  memcpy(&(ctx->long_state[i + 5 * 16]), &xmminput[5], 16);
  memcpy(&(ctx->long_state[i + 6 * 16]), &xmminput[6], 16);
  memcpy(&(ctx->long_state[i + 7 * 16]), &xmminput[7], 16);
#endif
  }
  ...
  // Load b into SIMD/plain representation
#if defined(AESNI)
  ...
  __m128i a_x;
#if defined(AESNI)
  c_x = _mm_load_si128((__m128i *)&ctx->long_state[a[0] & 0x1FFFF0]);
...
#else
  memcpy(c, &ctx->long_state[a[0] & 0x1FFFF0], 16);
#endif
    //__builtin_prefetch(&ctx->long_state[c[0] & 0x1FFFF0], 0, 1);

  // b_x ^= c_x; store into long_state
#if defined(AESNI)
  b_x = _mm_xor_si128(b_x, c_x);
  _mm_store_si128((__m128i *)&ctx->long_state[a[0] & 0x1FFFF0], b_x);
#else
  for (int k = 0; k < 16; ++k) {
    ctx->long_state[(a[0] & 0x1FFFF0) + k] ^= ((uint8_t*)c)[k];
  }
#endif
  ...
  for (i = 0; likely(i < MEMORY); i += INIT_SIZE_BYTE)
  {
    ...
    for (int blk = 0; blk < 8; ++blk) {
      for (int k = 0; k < 16; ++k) {
        ((uint8_t*)&xmminput[blk])[k] ^= ctx->long_state[i + blk * 16 + k];
      }
    }
#endif
